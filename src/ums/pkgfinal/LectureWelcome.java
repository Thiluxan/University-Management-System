...
public class LectureWelcome extends javax.swing.JFrame{
        ...
        try{
            ...
            try(Connection con = DriverManager.getConnection(
                ...
                if(evt.getSource() == jButton3) {
                    try {
                        username = u1;
                        try (PreparedStatement sts = con.prepareStatement("SELECT * FROM lec_user WHERE username = ? AND password = ?")) {
                            sts.setString(1, u1);
                            sts.setString(2, p1);
        ...
        try{
           Class.forName("com.mysql.jdbc.Driver");
            ...
            if(evt.getSource() == jButton2) {
                username = u2;
                try(PreparedStatement sta = con.prepareStatement("SELECT * FROM lec_user WHERE username = ?")){
                    sta.setString(1,u2);
                    ...
                    if(Arrays.equals(pw2,pw3)) {
                        ...
                        try (PreparedStatement sts = con.prepareStatement("INSERT INTO lec_user(username, password) VALUES (?, ?)")) {
                            sts.setString(1, u2);
                            sts.setString(2, p2);
                            sts.executeUpdate();
